				...
				<view class="ul">
					<view class="li dis_f_c_c" v-for="(li,li_index) in days" :key="li_index">
						<block v-for="(day,index) in li" :key="index">
							...
							<!-- 当前月 -->
							<template v-else>
								<view class="day dis_f_co_c_c box" :class="{'true':day.click,'today':day.nyr==today.nyr}" @click.stop="list_fun(day)">
									<text>{{day.r}}</text>
								</view>
							</template>
						</block>
					...
					li.forEach((item) => {
						...
						if (list.length) {
							...
							for (let j = 0; j < list.length; j++) {
								...
								let obj = list[j];
								if (key) {
									if (obj[key] == item.r || obj[key] == item.yr || obj[key] == item.nyr) {
										click = true;
										// 如果有key值，说明数据中需要其他字段，直接将数据赋值到这天的data中
										data = obj;
										break;
...
<style lang="scss" scoped>
	...
	.qian_dao .section .days .ul .day{
		border: none;
	}
	.qian_dao .section .weeks .weeks_ul .weeks_li{
		border: none;
	}
</style>
